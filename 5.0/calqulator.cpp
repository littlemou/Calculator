...
void Calqulator::hyp_equal_clicked()
{
    QString value=ui->lineEdit->text();
    QString x_str;
    double x_digital;
    ...
    for(int i=0;i<value.length();i++)
    {
        if(value[i]=='s')
        {
            for(int m=i+4;m<value.length();m++)
            {
                x_str.push_back(value[m]);
            }
            x_digital=compute(intopost(x_str));
            ...
            ui->lineEdit->setText(value+" = "+QString::number(sinh(x_digital),'g',6));
            break;
        }
        if(value[i]=='c')
        {
...
void Calqulator::specialoperator_h_clicked()
{
    ...
    disconnect(ui->equalbtn,&QPushButton::clicked,this,&Calqulator::equal_clicked);
    connect(ui->equalbtn,&QPushButton::clicked,this,&Calqulator::hyp_equal_clicked);
...
//        else
//        if((infix[j]=='-'||infix[j]=='+')&&(infix[j-1]<'0'||infix[j-1]>'9')&&infix[j-1]!='(')
//        {
//            int number=5;
//            QString str;
//            str.push_back("+(");
